...
def custom_read_func(data_paths, src_lang, trg_lang):
    for data_path in data_paths:
        with open(data_path, 'r', encoding='utf-8') as f:
            for line in f:
                src, tgt = line.strip('\n').split('\t')
                yield {src_lang: src, trg_lang: tgt}

#https://paddlenlp.readthedocs.io/zh/latest/data_prepare/dataset_self_defined.html
def create_data_loader(args, places=None):
    if args.train_file is not None and args.dev_file is not None:
        dataset_train = load_dataset(
            custom_read_func,
            data_paths=args.train_file,
            src_lang=args.src_lang,
            trg_lang=args.trg_lang,
            lazy=False)      
        dataset_dev = load_dataset(
            custom_read_func,
            data_paths=args.dev_file,
            src_lang=args.src_lang,
            trg_lang=args.trg_lang,            
            lazy=False)                   
        datasets = [dataset_train, dataset_dev]
    else:
        raise ValueError(
            "--train_file and --dev_file must not be None. ")
...
def create_infer_loader(args):
    if args.test_file is not None:
        dataset = load_dataset(
            custom_read_func,
            data_paths=args.test_file,
            src_lang=args.src_lang,
            trg_lang=args.trg_lang,            
            lazy=False)   
    else:
        raise ValueError(
            "--test_file must not be None. ")
